...
/*
 * KD-tree extraido de: https://rosettacode.org/wiki/K-d_tree#C
 */
int visited;
 
double dist(struct kd_node_t *a, struct kd_node_t *b)
{
	double t, d = 0;
	int i;
	for(i=0; i<3; i++) {
...
struct kd_node_t* make_tree(struct kd_node_t *t, int len, int i)
{
	struct kd_node_t *n;
...
int main(void)
{
	int i;
	struct kd_node_t wp[] = {
		{{2, 3, 4}}, {{5, 4, 7}}, {{9, 6, 8}}, {{4, 7, 2}}, {{8, 1, 4}}, {{7, 2, 4}}
	};
	struct kd_node_t testNode = {{9, 2, 5}};
	struct kd_node_t *root, *found, *million;
	double best_dist;
 
	root = make_tree(wp, sizeof(wp) / sizeof(wp[1]), 0);
